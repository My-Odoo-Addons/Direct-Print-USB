...
class PrintAgentGUI:
    ...
    def _create_widgets(self):
        ...
        # URL Odoo
        ttk.Label(config_frame, text="URL Odoo:").grid(row=0, column=0, sticky=tk.W, padx=(0, 10))
        self.odoo_url_var = tk.StringVar()
        odoo_entry = ttk.Entry(config_frame, textvariable=self.odoo_url_var, width=50, placeholder="http://host:port")
        odoo_entry.grid(row=0, column=1, sticky=(tk.W, tk.E), pady=5) 
        
        # Imprimante
        ttk.Label(config_frame, text="Imprimante:").grid(row=1, column=0, sticky=tk.W, padx=(0, 10))
        ...
        refresh_btn = ttk.Button(printer_frame, text="Rafraîchir", width=3, command=self._refresh_printers)
        ...
        self.start_btn = ttk.Button(button_frame, text="▶️ Démarrer l'Agent", 
                                    command=self._start_agent, style="success.TButton")
        self.start_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        self.stop_btn = ttk.Button(button_frame, text="⏹️ Arrêter l'Agent", 
                                   command=self._stop_agent, state=tk.DISABLED, style="danger.TButton")
        self.stop_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        test_btn = ttk.Button(button_frame, text="Test d'impression", 
        ...
        ttk.Label(total_frame, textvariable=self.total_var, font=("", 14, "bold")).pack()
        
        # Succès
        success_frame = ttk.Frame(stats_inner)
        success_frame.pack(side=tk.LEFT, expand=True, fill=tk.X)
        ttk.Label(success_frame, text="✓ Réussis", font=("", 9), foreground="green").pack()
        self.success_var = tk.StringVar(value="0")
        ttk.Label(success_frame, textvariable=self.success_var, 
                 font=("", 14, "bold"), foreground="green").pack()
        ...
        ttk.Label(error_frame, textvariable=self.error_var, 
                 font=("", 14, "bold"), foreground="red").pack()
        ...
        clear_btn = ttk.Button(log_frame, text="Effacer les logs", 
        ...
        try:
            if self.agent:
                ...
                try:
                    ...
                    self._log(f"✗ Impossible d'envoyer la demande d'arrêt: {e}", "error")
        except Exception:
            pass
    ...
    def _test_print(self):
        ...
        try:
            ...
            test_data += b"=" * 32 + b"\n"
            test_data += f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n".encode('cp437')
    ...
    def _update_status(self):
        ...
        if self.is_running:
            ...
            self.info_text.config(state=tk.NORMAL)
            self.info_text.delete(1.0, tk.END)
            self.info_text.insert(1.0, "Agent arrêté. Cliquez sur 'Démarrer' pour commencer.")
            self.info_text.config(state=tk.DISABLED)
...
class PrintAgentGUI_Wrapper:
    ...
    def __init__(self, odoo_url, printer_name, log_callback, stats_callback):
        ...
        self._command_queue = queue.Queue()  # queue pour les commandes depuis le thread GUI
                ...
                if data.get("type") == "print":
                    ...
                    if receipt_data:
                        if self.printer.print_raw(receipt_data):
                            ...
                            self.log_callback(f"✗ Échec impression: {order_name}", "error")
                            self.stats_callback("error")
                    else:
                        self.log_callback(f"✗ Ticket non récupéré: {order_name}", "error")
                        self.stats_callback("error")
            ...
            def _shutdown():
                async def _async_shutdown():
                    try:
                        ...
                        try:
                            self.log_callback(f"✗ Erreur lors de l'arrêt des serveurs: {e}", "error")
                        except Exception:
                            pass
