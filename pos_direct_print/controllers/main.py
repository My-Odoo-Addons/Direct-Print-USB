...
class PosDirectPrintController(http.Controller):
    ...
    @http.route(
        "/pos_direct_print/receipt/<path:order_name>",
        type="http",
        auth="public",
        csrf=False,
    )
    def get_receipt(self, order_name, **kwargs):
        ...
        order = (
            request.env["pos.order"].sudo().search([("name", "=", order_name)], limit=1)
        )
        ...
        try:
            ...
            return Response(
                ...
                headers={
                    ...
                    "X-Order-Date": (
                        order.date_order.isoformat() if order.date_order else ""
                    ),
                },
            )
    ...
    def status(self, **kwargs):
        ...
        return Response(
            json.dumps(
                {"status": "ok", "module": "pos_direct_print", "version": "18.0.1.0.0"}
            ),
            status=200,
            content_type="application/json",
        )
    ...
    @http.route(
        "/pos_direct_print/config/<int:config_id>",
        type="http",
        auth="public",
        csrf=False,
    )
            ...
            json.dumps(
                {
                    ...
                    "footer_message": config.direct_print_footer
    ...
    @http.route(
        "/pos_direct_print/test/<path:order_name>",
        type="http",
        auth="user",
        website=False,
    )
    ...
    def get_last_receipt(self, **kwargs):
        ...
        order = request.env["pos.order"].sudo().get_last_order(config_id=config_id)
